---
title: "10 Things to Know About Heterogeneous Treatment Effects^[This guide draws heavily from @gerber_green_2012 and from Don Green's course notes on experimental methods at Columbia University.]"
author: 
  - name: "Albert Fang"
    url: https://albertfang.com/
abstract: "This guide discusses methods for analyzing heterogeneous treatment effects: testing for heterogeneity, estimating subgroup treatment effects and their differences, and addressing the pitfalls of multiple comparisons and ad hoc specification search."
image: heterogeneous-effects.png
bibliography: heterogeneous-effects.bib
---

This guide discusses the theoretical and policy relevance of heterogeneous treatment effects. It also discusses and demonstrates methods for estimating them and interpreting results: from testing for heterogeneity, to estimating subgroup treatment effects and their differences, to addressing the pitfalls of multiple comparisons and ad hoc specification search.[^1]
...
A treatment may affect experimental subjects in different ways: this is treatment effect heterogeneity. The study of treatment effect heterogeneity is the study of these differences across subjects. For whom are there big effects? For whom are there small effects? For whom does treatment generate beneficial or adverse effects? 

#  Why is treatment effect heterogeneity important?
==

Investigating treatment effect heterogeneity can inform (or be used to test predictions of) our theories about the conditions under which treatments are especially effective or ineffective. It can also help in designing and deploying policies so as to maximize their effectiveness. If resources will be utilized to deploy a large-scale policy intervention, there are good arguments to be made for deploying it where we have reasonable evidence that it will work.


# Treatment effect heterogeneity: The general case

First, we might want to know if the treatment effect is the same for all subjects. Stated differently: whether or not the variance of the individual level treatment effect $\tau_i$ across all subjects is statistically distinguishable from zero. We can test the null hypothesis that $Var(\tau_i) = 0$. This is equivalent to the hypothesis of a constant treatment effect, i.e., $\tau_i = \tau, \forall i$. However, it is not possible to estimate $Var(\tau_i)$ in an experimental setting because you never get to see the treatment effect for any particular individual. Instead, you only get to see the outcome for each person either in the treatment or in the control condition.[^2]

What might this look like in practice? As an example, Eric Kramon, Sarah Brierley, George Ofosu in their study of [The Moderating Effect of Debates in Ghana](https://egap.org/resource/the-moderating-effect-of-debates-in-ghana/) uncover that individuals' partisanship condition the impact of viewing parliamentary debates on candidate evaluation and vote choice. In addition, the policy brief on [Reducing Clientelism in Benin](https://egap.org/resource/brief-13-reducing-clientelism-in-benin/) details how the effectiveness of different anti-clientelistic policies can depend on the incumbency status of candidates. In a third example, @blair2021 failed to find evidence of heterogeneous treatment effects of community policing interventions across six geographic sites in the Global South.

# Treatment effect heterogeneity: Conditional average treatment effects (CATEs)
==

Second, we might want to know if specific groups of experimental subjects are impacted differently by our treatment, on average. A structured, theory-driven inquiry of treatment effect heterogeneity involves pre-specifying and investigating conditional average treatment effects (CATEs). A CATE is is often considered to be an average treatment effect specific to a subgroup of subjects, where the subgroup is defined by subjects' attributes (e.g., the ATE among female subjects) or attributes of the context in which the experiment occurs (e.g., the ATE among subjects at a specific site in a multi-site field experiment). Though we do not focus on it in this guide, another version of a CATE can be specific to subjects' treatment status (e.g., the ATE among subjects who were assigned to treatment, also called the ATT or average treatment effect on the treated).

_An important aside on estimation_: When estimating CATEs that condition on attributes (of subjects or of their contexts), it's important that conditioning occurs only on pre-treatment covariates. One should avoid conditioning on variables whose values could have been affected by the treatment itself (called post-treatment variables). This is to ensure the unbiased estimation of conditional average treatment effects. Some researchers may be interested in post-treatment effect modification, or (in the regression context) the interaction between a treatment and a post-treatment covariate. For example, how do the effects of a job search assistance program vary with participants' levels of depression during the follow-up period? However, conditioning on a post-treatment covariate may lead to bias. See _cite angrist and pischke_ for more on so-called "bad" controls. There is a burgeoning body of methodological research on the conditions under which CATEs involving post-treatment covariates are identified. These methods rely on model-based identification.[^13]
...
Third, we might be interested in the difference between two CATEs (sometimes called an interaction effect). Does our treatment work differently on average for men and women? Stated differently, we might want to know whether the difference in ATEs between female and male experimental subjects is equal to (or different from) zero. Or perhaps we'd like to know whether the average treatment effect among rural study participants is different from the average treatment effect among urban participants.

_An aside on causal interpretation_: The variable(s) that define the subgroups that you compare may or may not have been experimentally manipulated. Much of the time, a treatment-by-covariate interaction can be interpreted as a _descriptive_ measure of association between the covariate and the treatment effect, but does not necessarily represent the _causal_ effect of a change in the covariate value on the ATE if the covariate is not randomly assigned.^[See @bansak2021 for more on causal interpretations of moderators.] Treatment-by-treatment interactions are differences in CATEs where the personal or contextual attribute defining subgroups _is_ experimentally manipulated. Because the covariate is randomly assigned, treatment-by-treatment interactions may be interpreted causally. Factorial and partial factorial designs allow researchers to randomly assign subjects to different combinations of "cross-cutting" treatment conditions and to estimate treatment-by-treatment interactions as allowed by the design.^[Note, however, that partial factorials may have issues. See @Muralidharanetal2020 for more on this.]

# Estimation
==

Estimating CATEs and differences between CATEs (interaction effects) is straightforward. For the CATE: estimate the ATE among subjects in the specific subgroup of interest. For differences in CATEs: take the difference between relevant estimated CATEs.

CATEs and differences in CATEs (interaction effects) may also be estimated in a regression framework. 

Imagine a hypothetical experiment evaluating the effect of a job training program on future earnings. Let $Y$ be the outcome (future earnings), $Z$ be the treatment variable (1=job training program, 0=control), and $X$ be a pre-treatment covariate (1=scholarship receipt, 0=no scholarship). The model
$$
\begin{aligned}
Y_i &= \alpha + \beta Z_i + \varepsilon_i \label{null}
\end{aligned}
$$
allows us to estimate the ATE ($\beta$).

We can modify this model in order to estimate the CATE and interaction effects. We can add an additional term interacting $Z$ and $X$, which yields
$$
\begin{aligned}
Y_i &= \alpha + \eta Z_i + \gamma X_i + \delta Z_iX_i + \varupsilon_i \label{alt}
\end{aligned}
$$.

This model allows us to back out the values of the CATEs. The ATE of the job training program among subjects who do not receive a scholarship is $\eta$. The ATE of the job training program among subjects who receive a scholarship is $(\beta + \delta)$. The ATE of the job training program among subjects who receive a scholarship is $(\eta + \delta)$.

The coefficient $\delta$ is the interaction effect and is interpreted as the difference between the ATE of the job training program among subjects receiving a scholarship and the ATE of the job training program among subjects not receiving a scholarship. This has a causal interpretation (i.e., $\delta$ is a treatment-by-treatment interaction) when scholarships are randomly assigned and a descriptive interpretation (i.e., $\delta$ is a treatment-by-covariate interaction) when scholarships are not randomly assigned.

An additional method used for covariate adjustment is that of @lin2013, which employs a  set of treatment by covariate interactions, with demeaned covariates.

# Hypothesis testing: CATEs and differences in CATEs
==

We might want to know whether an estimated CATE, or an estimated interaction effect, is just the result of noise in the data, rather than reflecting a true CATE or difference in CATEs.

To test whether the estimated interaction effect is just the result of noise in the data, rather than reflecting a true difference in CATEs, we can take one of two approaches. First, we can take the regression approach, by locating the standard error that corresponds to the interaction coefficient in one's regression output. This is common under a regression framework. For more on how to read regression table output, see [10 Things You Need to Know About Reading a Regression Table](https://methods.egap.org/guides/interpretation/regression-table_en.html). Second, one can use randomization inference. This entails generating a full schedule of potential outcomes under the null hypothesis that the true treatment effect is constant and equal to the estimated ATE. Then simulating random assignment a large number of times, and calculating how often the simulated estimate of the interaction effect is at least as large (in absolute value) as the actual estimate. For more on randomization inference, see [10 Things You Need to Know About Randomization Inference](https://methods.egap.org/guides/analysis-procedures/randomization-inference_en.html).

One can also combine these two methods, conducting randomization inference in a regression framework. One method suitable for two-sided tests involves using the $F$-statistic as the test statistic, where the null model is
$$
\begin{aligned}
...
_An aside on interpretation_: Here, it's important to distinguish between difference in significance and difference in effects. If, for example, our evidence suggests that the ATE among female study participants is significantly different from zero, but that the ATE among male participants isn't, this _doesn't_ necessarily mean that the two CATEs are different from each other.
...
As mentioned above, one might be interested in whether the variance of the treatment effect $\tau_i$ across subjects is different from zero and seek to test the null hypothesis that $Var(\tau_i) = 0$ (which is equivalent to the hypothesis of a constant treatment effect, i.e., $\tau_i = \tau, \forall i$). However, it is not possible to estimate $Var(\tau_i)$ in an experimental setting because you never get to see the treatment effect for any particular individual. Instead, you only get to see the outcome for each person either in the treatment or in the control condition.[^2]
...
Machine learning methods are useful to automate the search for systematic variation in treatment effects. These automated approaches are attractive because they minimize researchers' use of discretion in selecting and testing interactions, and are useful for conducting exploratory analyses. These analyses are rarely pre-specified due to their exploratory nature.
