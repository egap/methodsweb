...
abstract: |
  When researchers report that "The estimated average treatment effect is 5 (p=.02)," they are using a shorthand to say that the experimental results are not consistent with the idea that the treatment had no effects. This guide explains the pieces of a hypothesis test: null hypotheses, test statistics, reference distributions, p-values, and the errors that arise when using p-values to make decisions. It focuses on randomized experiments and research designs that approximate randomization.
---
...
Recall from [Ten Things to Know about Causal Inference](https://methods.egap.org/guides/causal-inference/causal-inference_en.html) that the counterfactual conceptualization of causality uses the idea of potential
...
```{r, eval=FALSE}
## This code installs a development version of RItools.
## To run it yourself, uncomment and execute manually.
# library(devtools)
# dev_mode(on = TRUE, path = "R-dev")
# renv::install("dgof")
# renv::install("hexbin")
