...
](https://methods.egap.org/guides/data-strategies/randomization_en.html).] Alternatively, one might realize that
...
t1_experiment_df <-
  t1_experiment_df |> 
  mutate(
    b_income_000_winsorized = 
      squish(b_income_000, quantile(b_income_000, c(0, .99)))
  )
...
RVR <- sandwich::vcovHC(fit, type <- 'HC0')[-1,-1]
...
t1attr1 <- lm(i_attrition ~ Z, data = t1_experiment_df)
