...
This is necessarily equivalent to the ATE estimand $E[Y_i(1)]-E[Y_i(0)]$ if missingness is independent of potential outcomes (MIPO), i.e. $Y_i(Z) \perp R_i(Z)$ (@gerber_green_2013).\footnote{A slightly stronger version of this assumption used in the literature is Missingness Completely at Random (MCAR) which holds further that $\textbf{X}_i \perp R_i(Z)$, where $\textbf{X}_i$ includes all pretreatment covariates.} Thus, to interpret $E[Y_i(1)|R_i(1)]- E[Y_i(0)|R_i(1)]$ as an unbiased estimator of the ATE, we must impose an assumption that MIPO. 
