...
This is necessarily equivalent to the ATE estimand $E[Y_i(1)]-E[Y_i(0)]$ if missingness is independent of potential outcomes (MIPO), i.e. $Y_i(Z) \perp R_i(Z)$ [@gerber_green_2012].\footnote{A slightly stronger version of this assumption used in the literature is Missingness Completely at Random (MCAR) which holds further that $\textbf{X}_i \perp R_i(Z)$, where $\textbf{X}_i$ includes all pretreatment covariates.} Thus, to interpret $E[Y_i(1)|R_i(1)]- E[Y_i(0)|R_i(1)]$ as an unbiased estimator of the ATE, we must impose an assumption that MIPO. 
...
Practically, one can test for differential attrition using randomization inference. For this demonstration, we use data from @eisenbarth2021can ([replication data](https://osf.io/td2p3/)). For the purposes of this exercise, we focus on the outcome of getting a penalty...

TODO: Need to reshape household survey from long to wide, assess attrition on the gotpenalty variable. Check to see if there are new households in baseline in the papers. Next, write a model with some covariates (can find in the Do file). Interact covariates with the treatment, get f-stat. Run RI to find a p-value.
...
Alternatively, if pre-treatment missingness is minimal (less than 10% of observations), or when working with very small datasets with few observations relative to the number of covariates and missingness indicators, @lin_et_al_2016 advocate imputing the (unconditional) mean of the observed pre-treatment covariate. Call this variable `Ximputed_mean`.
...
library(attrition)
R <- abs(1 - missing)
boundData <- as.data.frame(cbind(Yobs, Z, R))
estimator_ev(Y = Yobs, Z = Z, R = R, minY = min(Yobs, na.rm = T), maxY = max(Yobs, na.rm = T), data = boundData)[c(3,4)]
```
...
3. *Pooling estimates*: Finally, researchers combine estimates from the different imputed datasets to generate a point estimate of the ATE and its stanard error. Typically, this point estimate can be calculated using rules laid out by @rubin_2004. 
