...
[^2]:  Reproduced from @gerber_green_2012, page 93.
...
## Stylized income example, matching the age/gender story in Section 7:
## Y is income (in thousands). Older subjects earn more; men earn more
## than women; U is residual variation we do not observe.
N   <- 500
tau <- 2        # treatment effect: +$2k of income
...
design <-
  declare_model(
    ...
    U      = rnorm(N, sd = 5),
    potential_outcomes(Y ~ tau * Z + 0.5 * age + 10 * gender + U)) +
  declare_inquiry(ATE = mean(Y_Z_1 - Y_Z_0)) +
  declare_assignment(Z = complete_ra(N = N, m = N/2)) +
  declare_measurement(Y = reveal_outcomes(Y ~ Z)) +
  declare_estimator(Y ~ Z,                inquiry = "ATE",
                    .method = lm_robust,  label = "Simple") +
  declare_estimator(Y ~ Z + age + gender, inquiry = "ATE",
                    .method = lm_robust,  label = "Covariate Adjusted")

## Diagnose power at this N
alpha <- 0.05
study_diagnosands <- declare_diagnosands(power = mean(p.value <= alpha))
diag <- diagnose_design(design, diagnosands = study_diagnosands, sims = 500)

## Sweep over sample sizes for the graph in Section 7
...
## Preview the first few rows of the Simple-estimator sweep
kable_styling(kable(head(plotdf[plotdf$estimator == "Simple", c(2, 7)]),
                    row.names = FALSE))
```

The DeclareDesign code above is compact but abstract. Here is the same single-design power calculation written out explicitly --- draw baseline data, assign treatment, reveal outcomes, estimate, test, count --- so you can see what is happening under the hood:

```{r, message=FALSE, warning=FALSE, error=FALSE}
## Hand-rolled version of the "Simple" power estimate from the
## DeclareDesign run above. Fixed N; no covariate adjustment.
set.seed(1234)
sims     <- 500
alpha    <- 0.05
...
for (i in seq_len(sims)) {
  ...
  Y0 <- 0.5 * age + 10 * gender + U
  Y1 <- Y0 + tau
  ## Random assignment (complete randomization, m = N/2)
  Z <- sample(rep(c(0, 1), each = N / 2))
  ## Reveal the outcome we actually observe
  Y <- Y1 * Z + Y0 * (1 - Z)
  ## Estimate and test via simple difference in means
  p_values[i] <- summary(lm(Y ~ Z))$coefficients[2, 4]
}
mean(p_values <= alpha)
```

This number should match the "Simple" row of the DeclareDesign diagnosis above, within simulation error. The loop cleans up three small problems in older versions of this guide: it does not re-draw Y inside the inner loop, it uses complete random assignment (`sample(rep(c(0,1), each = N/2))`) rather than independent coin flips, and it reports the power estimate directly rather than printing a long vector.
...
```{r, message=FALSE, error=FALSE, warning=FALSE}
library(ggplot2)
## Show p-values from one estimator only; mixing "Simple" and
## "Covariate Adjusted" rows makes the histogram hard to read.
bardf <- diag$simulations_df[diag$simulations_df$estimator == "Simple", ]
ggplot(bardf, aes(x = p.value)) +
  geom_histogram(fill = "white", color = "black", bins = 100) +
  geom_vline(xintercept = 0.05, color = "red", linetype = "dotted", linewidth = 1) +
  labs(x = "p-value (Simple estimator)", y = "Number of Experiments") +
  theme_bw()
```
