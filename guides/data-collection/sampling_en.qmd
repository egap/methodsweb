...
Researchers are rarely able to collect measurements on all units that make up the target population of a study; instead, they select a subset of units --- a process called sampling. This guide provides an overview of different ways to sample and how these affect what can be learned from a study.
...
plot1 <- ggplot(plot_data, aes(x = estimates)) +
  geom_histogram(binwidth = 2, fill = "grey80", color = "black") +
  theme_bw() +
  xlab("Sample Estimate of Average Income") +
  ylab("Count") +
...
# Different ways of getting a random sample
...
# What to do without an obvious sampling frame
...
# Sampling difficult-to-reach populations
...
This model, however, has multiple limitations. For example, it presumes sampling with replacement, i.e., the model does not take into account that a respondent who has been sampled in a previous wave cannot be sampled again. @gile2018methods describe a range of subsequent approaches that seek to relax this and other assumptions.


# Keep sampling strategy simple and appropriate for local context


The practical implementation of any sampling strategy typically presents numerous obstacles. Many unforeseen problems can occur and these are often context-specific. Hence, pre-testing sampling strategies is important. Here are some examples of difficulties that one may be able to discover and address during a pre-test:
...
Again, the logic extends to cases in which the target of inference is a PATE. In randomized experiments, researchers often need to adjust their uncertainty estimates to reflect the additional variability induced by cluster random assignment (see ["10 Things to Know About Cluster Randomization"](https://methods.egap.org/guides/data-collection/cluster-randomization_en.html)). If the experimental subject pool is a random sample from a larger population, however, researchers may have to adjust for clustering at a level higher than that of treatment assignment if sampling was clustered at higher levels (see @abadie2017should).


# How sampling helps with problems of non-response
