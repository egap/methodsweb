...
While the original study uses PCA to construct the index, we also illustrate four other approaches: the simple average, factor analysis, a two-step graded-response IRT (item parameters are fit first, then the resulting respondent scores are regressed on the treatment), and a latent-regression IRT in which the treatment indicator enters the model directly as a predictor of the latent trait. We fit the IRT models with the `mirt` package [@chalmers2012mirt]; the latent-regression specification is closely related to the hierarchical IRT framework of [@zhou2020hirt].
...
score_ave <- rowMeans(items_std, na.rm=TRUE)
...
# IRT (two step) ############################################
# Fit a graded response model on the items, extract EAP scores,
# then regress the scores on the treatment indicator.
items_df <- as.data.frame(items)
out_irt <- mirt(items_df, model=1, itemtype="graded", verbose=FALSE)
score_irt <- fscores(out_irt, method="EAP")[,1]
data$score_irt <- -score_irt
ate_irt <- lm(score_irt ~ stockreal, data=data)

# Latent-regression IRT ####################################
# Fit the GRM with the treatment indicator entering directly as a
# predictor of the latent trait. Drop rows where all items are
# missing so mirt's covdata alignment works.
keep <- rowSums(!is.na(items)) > 0 & !is.na(data$stockreal)
out_hirt <- mirt(as.data.frame(items[keep,]),
                 model=1, itemtype="graded",
                 covdata=data.frame(stockreal=data$stockreal[keep]),
                 formula=~stockreal,
                 verbose=FALSE, SE=TRUE)
lr <- coef(out_hirt, printSE=TRUE)$lr.betas
# Sign flip to match the higher-score-is-right-wing convention used
# for the other four methods.
ate_hirt_est <- -lr$betas["stockreal","F1"]
ate_hirt_se  <-  lr$SE   ["stockreal","F1"]
```
```{r, fig.cap="**Comparison of the Average Treatment Effect with Different Methods to Construct the Index**: This figure compares the estimated average treatment effect (ATE) of investment experience on the outcome index constructed five different ways. The point is the estimated ATE; the horizontal line is the 95% confidence interval. The first four estimators construct an index from the items and then regress that index on the treatment indicator; the fifth (latent-regression IRT) fits the IRT model and the treatment effect jointly, with the treatment predicting the latent trait directly. All five methods yield a positive, statistically significant effect, consistent with the original study. The latent-regression IRT produces the largest point estimate and the smallest standard error, as one would expect: the two-step estimators regress noisy estimated scores on the treatment, which attenuates the slope, while the latent-regression model accounts for measurement uncertainty in the latent trait."}
#| echo: false
# Results ###################################################
par(mar=c(5, 8, 4, 2))
...
points(x=ate_hirt_est, y=5, pch=19)
lines(x=c(ate_hirt_est - 1.96*ate_hirt_se, ate_hirt_est + 1.96*ate_hirt_se), y=c(5, 5), lwd=2)
abline(v=0, lty=2)
